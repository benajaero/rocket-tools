...
def unit_convert(value: float, from_unit: str, to_unit: str) -> dict:
    """Convert between engineering units. Supports: m, mm, inch, ft, pa, kpa, mpa, psi, n, kn, lbf, c, k, f."""
...
def material_lookup(name: str, property_filter: str = "all") -> dict:
    """Look up aerospace material properties by name (e.g. 6061-T6, Ti-6Al-4V, 7075-T6, 4130, Inconel-718)."""
...
def beam_analysis(
    ...
    cross_section: {"type": "rectangle", "width": float, "height": float} or {"type": "circle", "diameter": float}
